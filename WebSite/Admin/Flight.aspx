                    ...
                    <div class="modal-body">
                       <div class="panel panel-default">
                           <div class="panel-body">
                               ...
                               <div v-if="isNew">
                                   <label>Id</label>
                                   <input type="text" v-model.lazy="model.Id" class="form-control"/>
                               </div>
                               <div v-else>
                               ...
                               <select v-model="model.RoutId" class="form-control" v-for="r in routeData">
                                  <option v-bind:value="`${r.Id}`">{{r.FromAirport}} : {{r.ToAirport}}</option>
                               </select>
                           </div>
                       </div>
                    </div>
        ...
        <hr />
            ...
            <tbody style="text-align:center;">
                <tr v-for="item, index in ajaxData">
                    <td>{{item.Id}}</td>
                    <td>{{item.RoutId}}</td>
                    <td>{{item.Route.FromAirport}}</td>
                    <td>{{item.Route.ToAirport}}</td>
    ...
    <script>
        var vue = new Vue({
            ...
            methods: {
                update: function () {
                    var scope = this;
                    axios.put('http://localhost:52014/api/FlightsApi/Put', this.model);
                ...
                del: function (id) {
                    var scope = this;
                    axios.delete(`http://localhost:52014/api/FlightsApi/${this.ajaxData[id].Id}`).then(function () {
                        scope.ajaxData.splice(id, 1);
                    });
                },
                post: function () {
                    axios.post('http://localhost:52014/api/FlightsApi/Post', this.model);
                    var scope = this;
                    var r = this.routeData.find(function (e) { return e.Id == scope.model.RoutId; })
                    console.log(r)
                    this.model.Route.FromAirport = r.FromAirport;
                    this.model.Route.ToAirport = r.ToAirport;
                    this.ajaxData.push(this.model);
                    console.log(this.model.RoutId);
                },
                loadAll: function () {
                    var scope = this;
                    axios.get('http://localhost:52014/api/FlightsApi', { headers: { accept: 'application/json' } }).then(function (data) {
                ...
                find: function (id) {
                    var scope = this;
                    axios.get(`http://localhost:52014/api/FlightsApi/${id}`, { headers: { accept: 'application/json' } }).then(function (data) {
                        var value = data.data;
                        if (value != null) {
                            scope.model = value;
                            scope.isNew = false;
