                    ...
                    <div class="modal-body">
                       <div class="panel panel-default">
                           <div class="panel-body">
                               ...
                               <input type="text" class="form-control" v-model="model.PassengerId" />
                               <label>Schedule Id</label>
                               <input type="text" class="form-control" v-model="model.ScheduleId" />
                           </div>
                       </div>
                    </div>
    ...
    <script>
        var vue = new Vue({
            ...
            methods: {
                update: function () {
                    var scope = this;
                    axios.put('http://localhost:52014/api/ReservationApi/Put', this.model);
                ...
                del: function (id) {
                    var scope = this;
                    axios.delete(`http://localhost:52014/api/ReservationApi/${this.ajaxData[id].Id}`).then(function () {
                        scope.ajaxData.splice(id, 1);
                    });
                },
                post: function () {
                    axios.post('http://localhost:52014/api/ReservationApi/Post', this.model);
                     this.ajaxData.push(this.model);
                },
                loadAll: function () {
                    var scope = this;
                    axios.get('http://localhost:52014/api/ReservationApi', { headers: { accept: 'application/json' } }).then(function (data) {
                ...
                find: function (id) {
                    var scope = this;
                    axios.get(`http://localhost:52014/api/ReservationApi/${id}`, { headers: { accept: 'application/json' } }).then(function (data) {
                        var value = data.data;
                        if (value != null) {
                            scope.model = value;
                        }
